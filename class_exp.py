...
op.add_option("--result_file_name",
              action="store", dest="result_file_name",
...
def benchmark(clf):
    ...
    fmicro = f1_score(y_test, pred, average='micro')
    print("f1-micro:   %0.3f" % score)

    fmacro = f1_score(y_test, pred, average='macro')
    print("f1-macro:   %0.3f" % score)
    ...
    return clf_descr, score, fmacro, fmicro, train_time, test_time
...
df.to_csv(opts.result_file_name)
